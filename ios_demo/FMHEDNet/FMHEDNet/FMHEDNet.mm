    ...
    if (tf_session.get() != nullptr) { // std::unique_ptr::get()
...
#else
        tensorflow::Status run_status = tf_session->Run({{kInputLayerName, input_tensor},
        ...
        if (run_status.ok() != true) {
            ...
            processImageError = [NSError errorWithDomain:FMHEDNetErrorDomain
                                                ...
                                                userInfo:@{
                                                        FMHEDNetProcessImageErrorKey: [NSString stringWithUTF8String:run_status.error_message().c_str()]}];
        } else {
            tensorflow::Tensor output = std::move(finalOutput.at(0));
            if (isDebug) {
                std::cout << "-- tensorflow::Tensor output dtype() is: " << output.dtype() << std::endl;
                std::cout << "-- tensorflow::Tensor output shape().DebugString() is: " << output.shape().DebugString()
                          << std::endl;
            ...
            if (isDebug) {
                std::cout << "debug output.dim_size: " << output.dim_size(1) << ", " << output.dim_size(2) << ", "
                          << output.dim_size(3) << std::endl;
            }
            ...
            cv::Mat outputMat = cv::Mat((int) output.dim_size(2), (int) output.dim_size(1), CV_32FC1,
